...
Somos el equipo de ciencia de datos de un proveedor de nube. Los datos de nuestros clientes (uso de servicios, facturación, tickets de soporte, etc.) llegan crudos, con errores, nulos y formatos inconsistentes. Nuestro trabajo es limpiarlos, ordenarlos y transformarlos para que tres áreas de la empresa —FinOps, Soporte y Producto— puedan usarlos sin complicaciones: información clara, confiable y fácil de consultar.

Hoy, mucha de esta información se arma de forma manual, lo que lleva tiempo y es propenso a errores. Con este proyecto buscamos automatizar ese proceso, combinando dos formas de entregar la información:
